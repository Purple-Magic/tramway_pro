...
class Courses::Topic < ApplicationRecord
  ...
  def progress_status
    ...
    if started_lessons.any? && started_lessons.count == lessons.active.count && started_lessons.count == done_lessons.count
      return :done
    end

    :in_progress if (lessons.active.any? && lessons_with_comments_any) || started_lessons.count != done_lessons.count
  end
end
