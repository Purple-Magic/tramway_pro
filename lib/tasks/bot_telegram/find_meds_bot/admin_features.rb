...
module BotTelegram::FindMedsBot::AdminFeatures
  ...
  def send_approve_message_to_admin_chat(offer)
    ...
    if offer.benchkiller_user&.company&.approved?
      ...
      text = ::FindMeds::Offers::AdminChatDecorator.decorate(offer).admin_message
      keyboard = [
        [
      ...
      send_notification_to_chat ::BotTelegram::FindMedsBot::ADMIN_CHAT_ID, message
    end
  end
...
host: Settings[Rails.env][:purple_magic])
  ...
  def approve_offer(argument)
    offer = ::FindMeds::Offer.find argument
    offer.approve
  end
